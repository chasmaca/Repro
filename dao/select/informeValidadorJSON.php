...
function recuperaListadoDetalleValidador($anio,$dpto,$subdpto,$tipoInforme){
	...
	if (!$informeResult) {
		...
		while($row=mysqli_fetch_array($informeResult)){
			...
			$tmp["departmento"] = utf8_decode($row['departamento']);
			$tmp["subdepartmento"] =  utf8_decode($row['subdepartamento']);
			$tmp["fecha"] = $row['fecha'];
			$tmp["encuadernacion"] = $row['encuadernacion'];
			$tmp["byn"] = $row['byn'];
			$tmp["color"] = $row['color'];
			$tmp["varios"] = $row['varios'];
			$tmp["nombre"] =  utf8_decode($row['nombre']);
			$tmp["apellidos"] =  utf8_decode($row['apellidos']);
			$tmp["descripcion"] =  utf8_decode($row['descripcion']);
