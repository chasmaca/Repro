...
function insertamosVarios2Extra($solicitud,$tipo,$descripcion,$unidades,$precio,$total){
	...
	if ($lineas == 0){
		...
		$lineas = recuperamosMaxVarios2Extra();
		$operacionVarios2 = insertamosVarios2($lineas,$tipo,$descripcion,$precio);
	...
	$lineasTrabajo = comprobamosOperacionTrabajoVarios2 ($solicitud,$lineas);
 	...
 	if ($lineasTrabajo == 0){
 		...
 		$operacionVarios2Trabajo = insertamosVarios2DetalleTrabajo($trabajo_id,$tipo,$lineas,$unidades,$solicitud,$total);
 		...
 		error_log($log, 3, "../../log/errores.log");
 		//$operacionVarios2Trabajo = true;
 		$operacionVarios2Trabajo = actualizamosVarios2DetalleTrabajo($trabajo_id,$tipo,$lineas,$unidades,$solicitud,$total);
 	}
...
function insertamosVarios2DetalleTrabajo($trabajo_id,$tipo,$detalle,$unidades,$solicitud,$total){
	global $mysqlCon;
	...
	$stmt->bind_param('iiiiid',$trabajo_id,$tipo,$detalle,$unidades,$solicitud,$total);
	if ($stmt->execute()){
		$operacion = true;
		$log  = "Right Query: ".$saveVarios2ExtraTrabajoJSON.PHP_EOL.
		"trabajo: ".$trabajo_id.PHP_EOL.
		"tipo: ".$tipo.PHP_EOL.
		"detalle_id: ".$detalle.PHP_EOL.
...
function comprobamosOperacionVarios2 ($descripcion,$precio){
	...
	"Function comprobamosOperacionVarios2".PHP_EOL.
...
function comprobamosOperacionTrabajoVarios2 ($solicitud,$lineas){
	global $mysqlCon;
	...
	if ($stmt = $mysqlCon->prepare($comprobarVarios2TrabajoExtraJSON)) {
		$stmt->bind_param('ii',$solicitud,$lineas);
		$stmt->execute();
		$log  = "Error Query: ".$comprobarVarios2TrabajoExtraJSON.PHP_EOL.
		"Function : comprobamosOperacionTrabajoVarios2".PHP_EOL.
		"solicitud: ".$solicitud.PHP_EOL.
		"detalle_id: ".$lineas.PHP_EOL.
		"-------------------------".PHP_EOL;
		error_log($log, 3, "../../log/errores.log");
