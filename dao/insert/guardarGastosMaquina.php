...
function insertamosGastosByN($departamento,$unidades,$precio,$total){
	...
	global $mysqlCon,$sentenciaInsertGastosMaquina,$mes,$anio,$fecha,$jsondata;
	...
	if (existeRegistro($departamento,$anio,$mes)){
		if ($stmt = $mysqlCon->prepare($sentenciaInsertGastosMaquina)) {
			$stmt->bind_param('isiddidd',$departamento,$fecha,$unidades,$precio,$total,$unidadesColor,$precioColor,$totalColor);
				
			if (!$stmt->execute()) {
				$jsondata["success"] = false;
				echo "Falló la ejecución: (" . $sentenciaInsertGastosMaquina->errno . ") " . $sentenciaInsertGastosMaquina->error;
			}else{
...
function actualizamosGastosByN($departamento,$unidades,$precio,$total,$anio,$mes){
	...
	global $mysqlCon,$sentenciaUpdateGastosMaquinaByN,$jsondata;

	if ($stmt = $mysqlCon->prepare($sentenciaUpdateGastosMaquinaByN)) {
		$stmt->bind_param('iddiss',$unidades,$precio,$total,$departamento,$anio,$mes);

		if (!$stmt->execute()) {
			$jsondata["success"] = false;
			echo "Falló la ejecución: (" . $sentenciaUpdateGastosMaquinaByN->errno . ") " . $sentenciaUpdateGastosMaquinaByN->error;
		}else{
			$jsondata["success"] = true;
		}
...
function actualizamosGastosColor($departamento,$unidades,$precio,$total,$anio,$mes){
	...
	global $mysqlCon,$sentenciaUpdateGastosMaquinaColor,$jsondata;

	if ($stmt = $mysqlCon->prepare($sentenciaUpdateGastosMaquinaColor)) {
		$stmt->bind_param('iddiss',$unidades,$precio,$total,$departamento,$anio,$mes);

		if (!$stmt->execute()) {
			$jsondata["success"] = false;
			echo "Falló la ejecución: (" . $sentenciaUpdateGastosMaquinaColor->errno . ") " . $sentenciaUpdateGastosMaquinaColor->error;
		}else{
			$jsondata["success"] = true;
		}
...
function existeRegistro($departamento,$anio,$mes){
	...
	global $mysqlCon,$existeGastoMaquina;
	$insertamos = true;

	/*Prepare Statement*/
	if ($stmt = $mysqlCon->prepare($existeGastoMaquina)) {
