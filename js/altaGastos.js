$(document).ready(function(){
	...
	var insertaGastosImpresora = function(periodo,departamento, unidades, precio, total,tipo) {

		return $.getJSON("../dao/insert/guardarGastosImpresora.php", {
	...
	var recuperaGastosImpresora = function(periodo){

		return $.getJSON("../dao/select/recuperaGastosImpresora.php",{
	...
	$('select').on('change', function() {
		...
		if (periodo != 0){
			...
			recuperaGastosImpresora(periodo).done(function(response) {
	...
	$('input.input').on('blur', function() {
		...
		if (numero !=""){
			...
			insertaGastosImpresora(periodo, departamento, numero, $(precioCampo).val(), $(valorCampo).val(),tipo)
